...
<template>
    ...
    <div class="min-h-screen bg-[#F0F7FF] font-sans p-6 md:p-12">
        <div class="max-w-6xl mx-auto ville-container">
            ...
            <div v-if="!ville && !isEditing" class="bg-white rounded-[3rem] p-20 text-center shadow-2xl shadow-blue-100 border-4 border-dashed border-blue-100">
                <div class="w-24 h-24 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-8">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-[#1DA1F2]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
                </div>
                <h2 class="text-4xl font-black italic uppercase text-slate-800 tracking-tighter mb-4">Votre aventure commence ici</h2>
                <p class="text-slate-400 font-bold uppercase tracking-widest mb-10">Vous n'avez pas encore configuré votre ville de jeu.</p>
                <Button @click="isEditing = true" class="!px-10 !py-5 !bg-[#1DA1F2] !border-none !rounded-2xl !shadow-xl">
                    <span class="text-white font-black italic uppercase tracking-widest text-lg">Créer ma Ville</span>
                </Button>
            </div>

            <!-- Case 2: Affichage de la Ville (Vue Prestige) -->
            <div v-if="ville && !isEditing" class="bg-white rounded-[4rem] shadow-2xl shadow-blue-100 overflow-hidden border-2 border-white">
                <div class="relative h-[500px]">
                    ...
                    <div class="absolute bottom-16 left-16 right-16">
                        <div class="flex items-center space-x-4 mb-6">
                            <span class="px-6 py-2 bg-yellow-400 text-white text-xs font-black uppercase rounded-2xl tracking-[0.2em] shadow-lg shadow-yellow-400/20">
                                {{ ville.lieux_count }} POINTS D'EXPLORATION
                            </span>
                            <span class="px-6 py-2 bg-white/10 backdrop-blur-md text-white text-xs font-black uppercase rounded-2xl tracking-[0.2em] border border-white/20">
                                {{ ville.pays }} • {{ ville.population?.toLocaleString() }} HAB.
                            </span>
                        </div>
                        <h1 class="text-8xl font-black italic uppercase text-white tracking-tighter leading-none mb-6">{{ ville.nom }}</h1>
                        <p class="text-white/80 text-xl font-bold max-w-3xl leading-relaxed italic">"{{ ville.description }}"</p>
                    </div>

                    <div class="absolute top-10 right-10 flex space-x-4">
                        <Button @click="isEditing = true" class="!p-4 !bg-white !text-[#1DA1F2] !border-none !rounded-2xl !shadow-2xl hover:!scale-110 transition-transform">
                            <template #default>
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            </template>
                        </Button>
                        <Link :href="route('admin.lieux.index')" class="p-4 bg-yellow-400 text-white rounded-2xl shadow-2xl shadow-yellow-400/40 hover:scale-110 transition-transform flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                            <span class="font-black italic uppercase tracking-widest">Gérer la carte</span>
                        </Link>
                    </div>
                </div>
                ...
                <div class="p-16 grid grid-cols-1 lg:grid-cols-3 gap-16">
                    <div class="lg:col-span-2 space-y-8">
                        <h3 class="text-3xl font-black italic uppercase text-[#1DA1F2] tracking-tighter">L'Histoire Secrète</h3>
                        <div class="text-slate-600 font-bold leading-loose text-lg whitespace-pre-line">
                            {{ ville.history }}
                        </div>
                    </div>
                    <div class="space-y-10">
                        <div class="bg-blue-50/50 rounded-[2.5rem] p-10 border-2 border-blue-50">
                            <h4 class="text-xl font-black italic uppercase text-slate-400 tracking-widest mb-6">Coordonnées GPS</h4>
                            <div class="space-y-4">
                                <div class="flex justify-between items-center border-b border-blue-100 pb-4">
                                    ...
                                    <span class="font-black text-[#1DA1F2]">{{ ville.latitude }}</span>
                                </div>
                                <div class="flex justify-between items-center pt-2">
                                    <span class="text-[10px] font-black uppercase text-slate-400">Longitude</span>
                                    <span class="font-black text-[#1DA1F2]">{{ ville.longitude }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div v-if="isEditing" class="bg-white rounded-[3rem] shadow-2xl overflow-hidden border-2 border-white">
                <div class="h-32 bg-[#1DA1F2] flex items-center px-12 justify-between">
                    <h2 class="text-4xl font-black italic uppercase text-white tracking-tighter">Configuration de la <span class="text-yellow-400">Cité</span></h2>
                    <Button icon="pi pi-times" @click="isEditing = false" class="!bg-white/10 !text-white !border-none !rounded-full" />
                </div>

                <form @submit.prevent="submit" class="p-12 md:p-16 space-y-10 font-sans">
                    ...
                    <div class="pt-6">
                        <Button type="submit" :loading="form.processing" class="w-full !py-6 !bg-[#1DA1F2] !border-none !rounded-3xl !shadow-xl !shadow-blue-200">
                            <span class="text-2xl font-black italic uppercase tracking-tighter">Enregistrer le Destin</span>
                        </Button>
                    </div>
                </form>
            </div>
